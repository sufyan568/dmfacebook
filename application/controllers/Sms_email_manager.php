...
class Sms_email_manager extends Home
{
    ...
    public function __construct()
    {
        ...
        if($function_name != "checking_open_rate") {
    ...
    { 
        ...
        if (!is_readable($csv)) {
            ...
            foreach ($data as $row) 
            {
                ...
                if($row[0] != '') $csv_data['first_name'] = $row[0];
                if($row[1] != '') $csv_data['last_name'] = $row[1];
                if($row[2] != '') $csv_data['phone_number'] = $row[2];
                if($row[3] != '') $csv_data['email'] = $row[3];

                if($row[3]=="email" && $row[2]=="phone_number") continue;
                ...
                if($row[2] != '' && $row[3] !='') {
                    ...
                    $or_where = array("email"=> $row[3], "phone_number" => $row[2]);

                    $sql = "(user_id ='".$user_id."' AND (phone_number='".$row[2]."' OR email='".$row[3]."'))";
                    $this->db->where($sql);
                    ...
                    if(count($db_data) > 0) {
                        ...
                        if(count($db_data) == 1) {
                            ...
                            if($row[2] == $db_data[0]['phone_number']) {

                                $this->basic->update_data("sms_email_contacts",array("user_id"=>$user_id,"phone_number"=>$row[2]), $csv_data);
                                $total_updated++;
                                continue;

                            } else {

                                $this->basic->update_data("sms_email_contacts",array("user_id"=>$user_id,"email"=>$row[3]), $csv_data);
                                $total_updated++;
                                continue;
                            }
                    ...
                    if($this->basic->is_exist("sms_email_contacts", array("user_id"=>$user_id,"phone_number"=>$row[2]))) {

                        $contactWithPhone = $this->basic->get_data("sms_email_contacts",array("where"=>array("user_id"=>$user_id,"phone_number"=>$row[2])));
                    ...
                    if($this->basic->is_exist("sms_email_contacts", array("user_id"=>$user_id,"email"=>$row[3]))) {

                        $contactWithEmail = $this->basic->get_data("sms_email_contacts",array("where"=>array("user_id"=>$user_id,"email"=>$row[3])));
                        $temp3 = $contact_group;
    ...
    public function email_campaign_lists_data()
    {
        ...
        $display_columns = array("#",'id','campaign_name','email_api','sent_count','posting_status','actions','number_of_unique_open','number_of_total_open','schedule_time','created_at');
        ...
        for($i = 0; $i < count($info); $i++)
        {
            ...
            if($info[$i]['schedule_time'] != "0000-00-00 00:00:00") {
                $info[$i]['schedule_time'] = "<div style='min-width:120px !important;' class='text-dark'>".date("M j, y H:i",strtotime($info[$i]['schedule_time']))."</div>";
            }
            else {
            ...
            if($info[$i]['number_of_unique_open'] == "1") {
                $info[$i]['number_of_unique_open'] = "<div class='text-dark'>".$this->lang->line('Yes')."</div>";
            } else {
                $info[$i]['number_of_unique_open'] = "<div class='text-muted'>".$this->lang->line('No')."</div>";
            }


            if($info[$i]['number_of_total_open'] != "0") {
                $info[$i]['number_of_total_open'] = "<div class='text-dark'>".$info[$i]['number_of_total_open']."</div>";
            } else {
                $info[$i]['number_of_total_open'] = "<div class='text-muted'>".$info[$i]['number_of_total_open']."</div>";
            }
            ...
            $report_btn = "<a href='#' class='campaign_report btn btn-circle btn-outline-primary' data-toggle='tooltip' title='".$this->lang->line("View Report")."' 
                table_id='".$info[$i]['id']."' 
                campaign_name='".$info[$i]['campaign_name']."',
                campaign_message='".htmlspecialchars($info[$i]['email_message'],ENT_QUOTES)."',
                attachment='".$forReport."',
    ...
    public function delete_email_campaign()
    {
        ...
        if($file_data[0]['email_attachment'] !="" || $file_data[0]['email_attachment'] !="Are you kidding???") {
    ...
    public function ajax_get_email_campaign_report_info()
    {
        ...
        $display_columns = array("#","id","contact_first_name","contact_last_name","contact_email","sent_time","delivery_id","is_open","number_of_time_open");
        ...
        for ($i=0; $i <count($info) ; $i++) { 
            ...
            if($info[$i]['is_open'] == "1")
                $info[$i]['is_open'] = "<div class='font-weight-bold text-dark'>".$this->lang->line('Yes')."</div>";
            else
                $info[$i]['is_open'] = "<div class='font-weight-bold text-muted'>".$this->lang->line('No')."</div>";
        }
    ...
    public function checking_open_rate($Campaign_table_id=0,$temp_table_id=0)
    {
        ...
        $this->basic->update_data("email_sending_campaign", array('id'=>$email_sending_campaign_table_id),array("number_of_unique_open"=>"1"));
        $increment_total_opened = "update email_sending_campaign set number_of_total_open=number_of_total_open+1 where id='{$email_sending_campaign_table_id}'";
        ...
        $this->basic->update_data("email_sending_campaign_send",array("id"=>$email_sending_campaign_send_table_id,"campaign_id"=>$email_sending_campaign_table_id),array("is_open"=>"1"));
        $number_of_clicked_increment_sql = "update email_sending_campaign_send set number_of_time_open=number_of_time_open+1 where id='{$email_sending_campaign_send_table_id}' AND campaign_id='{$email_sending_campaign_table_id}'";

        if($number_of_clicked_increment_sql != '') {
            $this->db->query($number_of_clicked_increment_sql);
        }
