...
class Cron_job extends Home
{
    ...
    public function sms_sending_command($api_key="")
    {
        ...
        foreach($campaign_info_fildered as $final_campaign_info)
        {
            ...
            $subscriber_contacts_join = array("messenger_bot_subscriber"=>"sms_sending_campaign_send.subscriber_id=messenger_bot_subscriber.id,left");
            $subscriber_contacts_select = array('sms_sending_campaign_send.*','messenger_bot_subscriber.id AS subscriberid','messenger_bot_subscriber.is_bot_subscriber','messenger_bot_subscriber.page_table_id');
            $where2['where']   = array("campaign_id"=>$campaign_id,"processed"=>"0","is_bot_subscriber"=>"1","page_table_id"=>$pageTableId);
            $subscriber_contacts = $this->basic->get_data("sms_sending_campaign_send",$where2,$subscriber_contacts_select,$subscriber_contacts_join,$number_of_sms_to_be_sent_in_try);

            if(!empty($subscriber_contacts)){
                foreach ($subscriber_contacts as $subscriber_index => $subscriber_val) {
                    array_push($campaign_contacts, $subscriber_val);
                }
            }
    ...
    public function email_sending_command($api_key="")
    {
        ...
        foreach($campaign_info_fildered as $info2)
        {
            ...
            $subscriber_contacts_join = array("messenger_bot_subscriber"=>"email_sending_campaign_send.subscriber_id=messenger_bot_subscriber.id,left");
            $subscriber_contacts_select = array('email_sending_campaign_send.*','messenger_bot_subscriber.id AS subscriberid','messenger_bot_subscriber.is_bot_subscriber','messenger_bot_subscriber.page_table_id');
            $where2['where']   = array("campaign_id"=>$campaign_id,"processed"=>"0","is_bot_subscriber"=>"1","page_table_id"=>$pageTableId);
            $subscriber_contacts = $this->basic->get_data("email_sending_campaign_send",$where2,$subscriber_contacts_select,$subscriber_contacts_join,$number_of_email_to_be_sent_in_try);
            ...
            foreach ($campaign_contacts as $contacts_details) 
            {
                ...
                if($contactid != "0") {
                    $formatted = $contactid.'-contact';
                    $unscubscribe_btn   = '<a href="'.base_url("home/unsubscribe/").urlencode($formatted).'/'.bin2hex($contact_email).'">'.$this->lang->line('Click to Unsubscribe').'</a>';
                } else if($subscriberId != "0") {
                    $formatted = $subscriberId.'-subscriber';
                    $unscubscribe_btn   = '<a href="'.base_url("home/unsubscribe/").urlencode($formatted).'/'.bin2hex($contact_email).'">'.$this->lang->line('Click to Unsubscribe').'</a>';
                }
    ...
    {       
        ...
        {            
            ...
            // echo "<pre>";print_r($messenger_content); echo "<pre>";
            // echo "<pre>";print_r($sms_content); echo "<pre>";
            // echo "<pre>";print_r($email_content); echo "<pre>";
            // exit();
